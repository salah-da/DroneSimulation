...
# ---- Robust HTTP with retry/backoff for 429/5xx ----

def _robust_get(url: str, params: Dict[str, Any], max_retries: int = 6, base_sleep: float = 0.5) -> List[list]:
    """GET with exponential backoff. Returns parsed JSON (list).
    Handles 429/5xx with jittered sleep, respects Retry-After if provided.
    """
    attempt = 0
    while True:
        try:
            ...
            if r.status_code == 429:
                ...
                if retry_after is not None:
                    ...
                    sleep_s = (base_sleep * (2 ** attempt)) * (1.0 + 0.25 * random.random())
            ...
            # Network/HTTP errors: backoff and retry
            attempt += 1
            if attempt > max_retries:
                raise
            sleep_s = (base_sleep * (2 ** (attempt - 1))) * (1.0 + 0.25 * random.random())
            time.sleep(min(sleep_s, 10.0))
...
def fetch_last_n_klines(symbol: str, interval: str, n: int, futures: bool=True, rate_sleep: float=0.2) -> pd.DataFrame:
    ...
    Pagination advances by **openTime + interval_ms** to avoid duplicates.
    Deduplicates rows by openTime before building DataFrame.
    ...
    while remain > 0:
        ...
        batch = _robust_get(base, params=params)
        if not batch:
            break
        rows.extend(batch)

        # Advance by **openTime + interval_ms** to avoid overlap
        last_open = int(batch[-1][0])  # open time
        next_start = last_open + tf_ms
        if next_start <= cur:
            next_start = cur + tf_ms

        got = len(batch)
        remain -= got
        cur = next_start
        # Gentle pacing to be rate-limit friendly
        time.sleep(rate_sleep)

    if not rows:
        raise RuntimeError("No klines fetched.")

    # Deduplicate by openTime (row[0]) BEFORE DataFrame, keep last occurrence
    seen = set()
    uniq_rows = []
    for r in rows:
        t0 = int(r[0])
    ...
    df.index.name = "time"
    # Sort and dedup index as belt-and-suspenders
    df = df[~df.index.duplicated(keep="last")].sort_index()

    # Sanity: enforce strictly increasing timestamps
    if not np.all(np.diff(df.index.view('i8')) > 0):
        df = df[~df.index.duplicated(keep="last")].sort_index()
...
with st.sidebar:
    ...
    st.divider()
    # NEW: temporal non-maximum suppression of near-duplicate paths
    SEP_BARS = st.number_input("Min separation between matches (bars)", 0, 500, 10, 1,
                               help="Suppress near-duplicates: keep only one match per ±SEP_BARS window")
...
for tpl in winners:
    ...
    if PRICE_METHOD == "Cosine":
        ...
        if np.isnan(s_price) or s_price < THETA_PRICE:
            continue
        s_price_percent = s_price * 100.0  # keep sign for clarity
    else:  # DTW
    ...
    matches.append({
        "t_idx": tpl["t_idx"],                # keep t index for NMS separation
        "time": tpl["time"],
...
def nms_by_time(df_side: pd.DataFrame, sep_bars: int) -> pd.DataFrame:
    """Greedy non-maximum suppression by t_idx distance (±sep_bars).
    Ranks by s_price, then s_vel (if any), then |delta|.
    """
    if df_side.empty or sep_bars <= 0:
        return df_side
    sort_keys = ["s_price"]; ascending = [False]
    if "s_vel" in df_side.columns and df_side["s_vel"].notna().any():
        sort_keys.append("s_vel"); ascending.append(False)
    # prefer larger forward move magnitude
    df_side = df_side.assign(_abs_delta=df_side["delta"].abs())
    sort_keys.append("_abs_delta"); ascending.append(False)
    ranked = df_side.sort_values(sort_keys, ascending=ascending).reset_index(drop=True)

    kept_rows = []
    used_t = []
    for _, row in ranked.iterrows():
        t = int(row["t_idx"])
        if all(abs(t - u) >= sep_bars for u in used_t):
            kept_rows.append(row)
            used_t.append(t)
    out = pd.DataFrame(kept_rows).drop(columns=["_abs_delta"], errors="ignore")
    return out

# Build DataFrame and apply NMS per side
if not matches:
...
# Drop exact duplicate (time, side) just in case
m = m.drop_duplicates(subset=["time","side"], keep="last")

m_long_raw  = m[m["side"]=="LONG"]
m_short_raw = m[m["side"]=="SHORT"]

m_long  = nms_by_time(m_long_raw,  int(SEP_BARS))
m_short = nms_by_time(m_short_raw, int(SEP_BARS))

# Recombine after suppression
m = pd.concat([m_long, m_short], ignore_index=True)
...
if m.empty:
    st.warning("No matches after near-duplicate suppression. Try reducing 'Min separation between matches (bars)'.")
    st.stop()
...
for idx, (title, df_side) in enumerate([("LONG", m_long), ("SHORT", m_short)]):
    with ov_cols[idx]:
        ...
        # sort by s_price desc
        for _, row in df_side.sort_values("s_price", ascending=False).head(n_here).iterrows():
            label = f"{row['time']} (shape {row['s_price_%']:.0f}%)"
            # Ensure unique legend keys
            if label in overlays:
                k = 2
                while f"{label} [{k}]" in overlays:
                    k += 1
                label = f"{label} [{k}]"
            overlays[label] = mean_center(row["P_tpl"])
    ...
    for idx, (title, df_side) in enumerate([("LONG", m_long), ("SHORT", m_short)]):
        with ov_cols_v[idx]:
            ...
            for _, row in sorted_side.iterrows():
                ...
                label = f"{row['time']} ({'vel' if rank_key=='s_vel' else 'price'} {disp:.0f}%)"
                if label in overlays_v:
                    k = 2
                    while f"{label} [{k}]" in overlays_v:
                        k += 1
                    label = f"{label} [{k}]"
                overlays_v[label] = tpl_vel_centered
